...
function createCastHTML(cast) {
   ...
   const castList = cast.slice(0, 10).map(member => {
      const profilePath = member.profile_path
         ? `${IMAGE_BASE_URL}${PROFILE_SIZE}${member.profile_path}`
...
function createRecommendationsHTML(recommendations) {
   ...
   const cards = recommendations.slice(0, 10).map(item => {
      const title = item.title || item.name;
      const posterPath = item.poster_path
         ? `${IMAGE_BASE_URL}${POSTER_SIZE}${item.poster_path}`
         : 'placeholder.png';
...
function createProvidersHTML(providersData) {
   ...
   return `
      <div class="detail-section providers-section">
         ...
         <div class="providers-list">${providerIcons}</div>
      </div>`;
}
...
async function loadDetails(itemId) {
   ...
   try {
      const [itemData, creditsData, recommendationsData, providersData] = await Promise.all([
         ...
         fetchProxy(`/${currentType}/${itemId}/watch/providers`)
      ]);
      ...
      if (item.episode_run_time?.length > 0) runtime = `${item.episode_run_time[0]} min (aprox)`;
      ...
      modalBody.innerHTML = `
         ...
         <div class="modal-details-grid">
            <div class="detail-section">
               ...
               ${createProvidersHTML(providersData)}
            </div>
...
searchInput.addEventListener('input', (e) => {
   ...
   searchTimeout = setTimeout(() => loadItems(e.target.value.trim()), 500);
});
...
movieGrid.addEventListener('click', (e) => {
   ...
   if (card && !card.classList.contains('skeleton-card')) {
      loadDetails(card.getAttribute('data-id'));
   }
});

// Fechar modal — botão X
document.addEventListener('click', (e) => {
   if (e.target.closest('#closeModal')) movieModal.style.display = 'none';
});
